...
// 先切割再拼接，直到拼接到 chunkSize 左右
// 最后一个超过了chunkSize，但被完整保留了，所以 ChracterTextSplitter 不明智
console.log(logChunks);

// 不同模型有不同的编码器，需要根据模型名获取编码器，gpt-4 用 cl100k_base
const enc = getEncoding("cl100k_base");
logChunks.forEach(doc => {
    console.log(doc);
    console.log('characte length', doc.pageContent.length);
